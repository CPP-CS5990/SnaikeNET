...
def test__game_state_move_players():
    ...
    directions: list[Direction] = [Direction.NORTH, Direction.SOUTH, Direction.EAST, Direction.WEST, Direction.NORTH]
    player_initial_positions = [game_state._players[uid].get_head_position() for uid in player_uids]
    ...
    for i, uid in enumerate(player_uids):
        game_state._players[uid]._direction = directions[i]
        logger.info(
            f"Player {i + 1} initial position: {game_state._players[uid].get_head_position()}"
        )
    ...
    player_new_positions = [game_state._players[uid].get_head_position() for uid in player_uids]
    ...
    for i, uid in enumerate(player_uids):
        assert game_state._players[uid].get_length() == 1, f"Player {i + 1} length changed after moving!"
        assert game_state._players[uid].get_tail_position() == game_state._players[uid].get_head_position(), f"Player {i + 1} tail position is not the same as head position after moving!"
...
def test__game_state_move_players_eat_food():
    ...
    for i, uid in enumerate(player_uids):
        assert game_state._players[uid].get_length() == 2, f"Player {i + 1} length did not increase after eating food!"
        assert game_state._players[uid].get_tail_position() == player_initial_positions[i], f"Player {i + 1} tail position is not the same as initial position after eating food!"
