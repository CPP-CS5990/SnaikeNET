...
def test__game_state_player_initialization():
    ...
    for i in range(5):
        ...
        assert len(game_state.players) == i + 1, (
            f"Expected {i + 1} players, but got {len(game_state.players)}"
        )
    ...
    for i, uid in enumerate(player_uids):
        logger.info(
            f"Player {i + 1} initial position: {game_state.players[uid].get_head_position()}"
        )
    ...
    for i in range(len(positions)):
        for j in range(i + 1, len(positions)):
            ...
            assert distance >= 10, (
                f"Players {i + 1} and {j + 1} are too close together (distance: {distance:.2f})"
            )
