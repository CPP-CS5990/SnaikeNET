...
class SnaikenetClient:
    ...
    def __init__(self, on_receive_game_state_frame: Callable[[ClientGameStateFrame]] = lambda _: None,
                 server_tcp_port: int = 8888, server_host: str = 'localhost', send_interval_ms: int = 50):
    ...
    async def connect(self):
        ...
        reader, writer = await asyncio.open_connection(self._server_host, self._server_tcp_port)
        logger.debug(f"Sending registration message to server at {self._server_host}:{self._server_tcp_port}")
        ...
        if udp_port is not None:
            ...
            loop = asyncio.get_running_loop()
            await loop.create_datagram_endpoint(lambda: self._UdpProtocol(self), remote_addr=(self._server_host, udp_port))
            self._server_udp_port = udp_port
        else:
            logger.error("Failed to connect to server: No UDP port received for hole punching")
            raise ConnectionError("Failed to connect to server: No UDP port received for hole punching")

        # UDP hole punching:
        logger.debug(
            f"Sending UDP hole-punching datagram to server at {self._server_host}:{self._server_udp_port} with UUID {self._client_uuid}")
        for _ in range(5):  # Send multiple times to increase chances of successful hole punching
            await asyncio.sleep(0.1)
            self._udp_transport.sendto(self._to_json({"type": "hole_punch", "uuid": self._client_uuid}))
        ...
        if response_json.get("status") == "registered":
            logger.info(f"Successfully connected to server with UUID {self._client_uuid}")
            self._server_udp_port = udp_port
        else:
            logger.error(f"Failed to connect to server: {response_json.get('error', 'Unknown error')}")
            raise ConnectionError(f"Failed to connect to server: {response_json.get('error', 'Unknown error')}")
    ...
    def _send_direction(self):
        ...
        logger.debug(f"Sending direction {self._direction} to server at {self._server_host}:{self._server_udp_port} via UDP")
    ...
    class _UdpProtocol(asyncio.DatagramProtocol):
        ...
        def connection_made(self, transport: asyncio.DatagramTransport):
            logger.info("UDP connection established with server binding to local port {}".format(
        ...
        def datagram_received(self, data: bytes, addr: tuple[str, int]):
            ...
            try:
                ...
                logger.error(f"Failed to decode game state frame from server: {data.decode().strip()}")
                return
